...
<div class="space-y-3">
		...
		<div class="flex items-center gap-2">
			...
			{:else}
				<Select.Root type="single" value={selected} onValueChange={handleChange}>
					...
					<Select.Content class="max-h-72">
						<Select.Item value="default">默认（遵循应用主题）</Select.Item>
						<Select.Group>
							<Select.GroupHeading>黑暗</Select.GroupHeading>
							{#each darkThemes as t}
								<Select.Item value={t.id}>{t.label}</Select.Item>
							{/each}
						</Select.Group>
						<Select.Group>
							<Select.GroupHeading>光</Select.GroupHeading>
	...
	<p class="text-xs text-muted-foreground">适用于所有代码编辑器（compose 文件、环境变量、配置文件）。无论应用的浅色/深色主题设置如何，均遵循此主题。</p>
</div>
