		...
		{#each destinations as dest}
			...
			<Select.Item value={String(dest.id)}>
				<span class="flex items-center gap-2">
					...
					{dest.name}
					{#if localOnRemote}
						<span class="rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-amber-600 dark:text-amber-400" title="只有当此环境的 Docker 守护程序与 Dockhand 位于同一主机上时，本地路径仓库才能正常工作。">needs same host</span>
					{/if}
				</span>
			</Select.Item>
		{/each}
