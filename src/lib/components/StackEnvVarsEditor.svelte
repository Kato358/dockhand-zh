					...
					<div class="flex items-center h-9 w-5 justify-center shrink-0">
						{#if source === 'file'}
							<Tooltip.Root>
								...
								<Tooltip.Content side="bottom"><p class="whitespace-nowrap">从仓库中的 env 文件</p></Tooltip.Content>
							</Tooltip.Root>
						...
						{#if variable.key}
							{#if isVarRequired && !isVarMissing}
								<Tooltip.Root>
									...
									<Tooltip.Content><p>已定义必需变量</p></Tooltip.Content>
								</Tooltip.Root>
							{:else if isVarOptional}
								<Tooltip.Root>
									...
									<Tooltip.Content><p>可选变量（有默认值）</p></Tooltip.Content>
								</Tooltip.Root>
							{:else if isVarUnused}
								<Tooltip.Root>
									<Tooltip.Trigger>
			...
			<div class="text-center py-6 text-muted-foreground">
				...
				{#if !readonly}
					<Button type="button" variant="link" onclick={addVariable} class="mt-1 text-xs">
						<Plus class="w-3 h-3" />添加你的第一个变量</Button>
				{/if}
			</div>
		{/if}
	</div>
