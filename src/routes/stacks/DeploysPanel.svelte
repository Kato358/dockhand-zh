	...
	async function deleteRun(run: DeployRun) {
		...
		try {
			...
			if (!res.ok) {
				...
				toast.error(data?.error || '删除部署运行失败');
				return;
			}
			...
			l.delete(run.id);
			logs = l;
			toast.success('部署运行已删除');
		} catch {
			toast.error('删除部署运行失败');
		} finally {
	...
	>
		{#snippet emptyState()}
			<div class="flex min-h-[40vh] flex-col items-center justify-center py-10 text-center">
				...
				<p class="text-sm text-muted-foreground">尚未进行任何部署运行。</p>
				<p class="mt-1 text-xs text-muted-foreground">部署此编排——其运行历史记录显示在此处。</p>
			</div>
		{/snippet}
		...
		{#snippet expandedRow(run, _rowState)}
			...
			<div class="w-full p-4 pl-12 shadow-inner bg-muted text-xs">
				<div class="grid grid-cols-2 gap-x-4 gap-y-1.5 sm:grid-cols-3">
					...
					<div>
						<div class="text-muted-foreground">由…触发</div>
					...
					<div class="mt-2 space-y-1">
						{#if view.imagesBuilt.length > 0}
							<div class="flex flex-wrap items-center gap-x-3 gap-y-1">
								<span class="text-muted-foreground">已建成：</span>
						...
						{#if view.imagesPulled.length > 0}
							<div class="flex flex-wrap items-center gap-x-3 gap-y-1">
								<span class="text-muted-foreground">已拉出：</span>
				...
				<div class="mt-3">
					{#if panelState.truncated}
						<div class="mb-1">
							<span class="inline-flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-medium text-amber-600 dark:text-amber-400">
								<AlertTriangle class="h-2.5 w-2.5" />日志截断</span>
						</div>
					{/if}
					{#if panelState.logMissing || logEntry?.status === 'missing'}
						...
						<div class="rounded-md border border-dashed border-zinc-300 px-3 py-6 text-center text-muted-foreground dark:border-zinc-700">该协议已不再可用。</div>
