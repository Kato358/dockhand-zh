...
<div class="w-56 border-r p-3 overflow-y-auto shrink-0">
	<h3 class="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">地点</h3>
	<div class="space-y-1">
		...
		{#if defaultBasePath}
			...
			>
				...
				<span class="truncate" title={defaultBasePath}>Dockhand默认</span>
			</button>
		{/if}
		...
		{#if !defaultBasePath && locations.length === 0}
			<p class="text-xs text-muted-foreground italic px-2">暂无位置信息。请浏览文件夹并在此处添加。</p>
		{/if}
	</div>
</div>
