	...
	const willHappen = $derived.by(() => {
		...
		if (hasVolumes) {
			...
			lines.push(removeVolumes
				? { delete: true, kind: 'volumes', text: `删除名为 ${n} 的卷——数据无法还原` }
				: { delete: false, kind: 'volumes', text: `保留 ${n} 个命名卷` });
		}
		return lines;
	});
...
<Dialog.Root bind:open>
	<Dialog.Content class="max-w-2xl">
		<Dialog.Header>
			...
			<Dialog.Description>选择要移除的内容。编排中的容器始终会被停止并移除。</Dialog.Description>
		</Dialog.Header>

		{#if loading}
			<div class="flex items-center gap-2 py-4 text-sm text-muted-foreground">
				<Loader2 class="w-4 h-4 animate-spin" />检查哪些内容可以移除…</div>
		{:else if preview}
			<div class="my-1 space-y-3">
				{#if hasFiles}
					<label class="flex items-start gap-2.5 cursor-pointer">
						...
						<div class="min-w-0">
							<div class="text-sm">删除磁盘上的文件</div>
							{#if preview.stackDir}
				...
				{#if hasVolumes}
					<label class="flex items-start gap-2.5 cursor-pointer">
						...
						<div class="min-w-0">
							<div class="text-sm">删除指定名称的卷<span class="text-muted-foreground">(data is unrecoverable)</span></div>
			...
			<div class="mt-2 rounded-md border bg-muted/30 p-3">
				<div class="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">将会发生什么？</div>
