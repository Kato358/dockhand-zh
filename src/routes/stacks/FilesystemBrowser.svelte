						...
						{#each filteredEntries as entry}
							...
							>
								...
								{#if highlighted && isAdoptMode}
									<Badge variant="secondary" class="text-xs">Compose 文件</Badge>
			...
			<Dialog.Footer class="border-t pt-4">
				{#if selectMode === 'directory'}
					<div class="flex-1 flex items-center gap-2 min-w-0">
						<span class="text-xs text-muted-foreground shrink-0">已选：</span>
				...
				{:else}
					<div class="flex-1 text-xs text-muted-foreground">单击文件即可选中它</div>
				{/if}
				<Button variant="outline" onclick={handleClose}>取消</Button>
				{#if selectMode === 'directory'}
					<Button onclick={handleConfirm}>
						<FolderPlus class="w-4 h-4" />选择</Button>
				{:else if selectMode === 'file_or_directory'}
					<Button onclick={handleConfirm}>选择</Button>
				{:else}
					<Button
