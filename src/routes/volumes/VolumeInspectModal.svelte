...
<Dialog.Root bind:open>
	<Dialog.Content class="max-w-4xl max-h-[90vh] flex flex-col" onOpenAutoFocus={(e) => e.preventDefault()}>
		<Dialog.Header class="shrink-0">
			<ModalHeader icon={HardDrive} title="存储卷" name={volumeName}>
				{#snippet extra()}
			...
			{:else if volumeData}
				...
				<div class="space-y-3">
					...
					<div class="grid grid-cols-2 gap-3 text-sm">
						...
						<div>
							<p class="text-muted-foreground">已创建</p>
				...
				<!-- Mountpoint -->
				<div class="space-y-2">
					<h3 class="text-sm font-semibold">山顶</h3>
					<div class="p-2 bg-muted rounded">
						<code class="text-xs break-all">{volumeData.Mountpoint}</code>
					</div>
					<p class="text-xs text-muted-foreground">卷数据存储在主机上的位置</p>
				</div>
				...
				{:else}
					<div class="space-y-2">
						<h3 class="text-sm font-semibold">驱动选项</h3>
						<p class="text-sm text-muted-foreground">未配置任何驱动程序选项</p>
					</div>
				{/if}
				...
				<div class="p-3 bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-800 rounded">
					<p class="text-xs text-yellow-800 dark:text-yellow-200">
						<strong>笔记：</strong> Removing this volume will permanently delete all data stored in it.
