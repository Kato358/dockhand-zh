			...
			<div class="space-y-2 text-xs">
				{#if systemContainer === 'dockhand'}
					...
					<p class="text-muted-foreground">
						...
						update it from <a href="/settings?tab=about" class="text-primary hover:underline">设置 &gt; 关于</a>.
					</p>
				{:else}
					<p class="font-medium text-blue-600 dark:text-blue-400">自动更新不可用</p>
					<p class="text-muted-foreground">Hawser代理必须在其远程主机上进行更新。</p>
					<a
					...
					>
						<ExternalLink class="w-3 h-3" />请在 GitHub 上查看更新说明。</a>
				{/if}
			</div>
		</div>
	</div>
		...
		{#if enabled}
			...
			{#if envHasScanning}
				<div class="space-y-1.5">
					<Label class="text-xs font-medium">脆弱性标准</Label>
					<VulnerabilityCriteriaSelector
						bind:value={vulnerabilityCriteria}
						onchange={(v) => oncriteriachange?.(v)}
					/>
					<p class="text-xs text-muted-foreground">如果新镜像存在符合此条件的漏洞，则阻止自动更新。</p>
				</div>
			{/if}
		{/if}
	</div>
