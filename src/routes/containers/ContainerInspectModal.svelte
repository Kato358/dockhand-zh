	...
	async function connectToNetwork() {
		...
		try {
			...
			if (response.ok) {
				...
				toast.success(`已连接到 ${net?.name || '网络'}`);
				...
				{#if containerData && !loading}
					<div class="ml-auto mr-6 flex items-center gap-1">
						...
						<Button
							...
							class="ml-1"
						>
							<Code class="w-4 h-4 mr-1.5" />检查</Button>
					</div>
				{/if}
			</Dialog.Title>
		</Dialog.Header>
			...
			{:else if containerData}
				<Tabs.Root bind:value={activeTab} class="w-full h-full flex flex-col">
					<Tabs.List class="w-full justify-start shrink-0 flex-wrap h-auto min-h-10 bg-muted rounded-lg">
						...
						<Tabs.Trigger value="processes" onclick={() => { showLogs = false; if (processesAutoRefresh) startProcessesCollection(); else fetchProcesses(); }}>流程</Tabs.Trigger>
						...
						<Tabs.Trigger value="env" onclick={() => showLogs = false}>环境</Tabs.Trigger>
						...
						{#if containerData.State?.Running}
							<div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
								...
								<div class="p-3 border border-border rounded-lg">
									...
									<div class="space-y-1 text-xs">
										<div class="flex justify-between">
											<span class="text-muted-foreground">读：</span>
											<span class="font-mono">{formatBytes(currentStats?.blockRead ?? 0)}</span>
										</div>
										<div class="flex justify-between">
											<span class="text-muted-foreground">写：</span>
								...
								<div class="p-3 border border-border rounded-lg">
									<div class="flex items-center gap-2 mb-2">
										...
										<span class="text-xs font-medium">流程</span>
										<button
											type="button"
						...
						<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
							...
							<div class="space-y-3">
								...
								<div class="grid grid-cols-2 gap-2 text-sm">
									...
									<div>
										<p class="text-muted-foreground text-xs">退出代码</p>
							...
							<div class="space-y-3">
								...
								<div class="grid grid-cols-2 gap-2 text-sm">
									...
									<div>
										<p class="text-muted-foreground text-xs">已创建</p>
										<p class="text-xs">{formatDate(containerData.Created)}</p>
									</div>
									<div>
										<p class="text-muted-foreground text-xs">已启动</p>
					...
					<Tabs.Content value="processes" class="overflow-auto data-[state=inactive]:hidden">
						...
						{:else}
							<p class="text-sm text-muted-foreground">未找到任何进程</p>
						{/if}
					</Tabs.Content>
					...
					<Tabs.Content value="network" class="space-y-4 overflow-auto">
							...
							<div class="space-y-2">
								...
								<div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
									{#if containerData.HostConfig?.Dns?.length > 0}
										<div class="p-2 bg-muted rounded">
											<p class="text-xs text-muted-foreground mb-1">DNS服务器</p>
						...
						<!-- Networks -->
						<div class="space-y-2">
							<h3 class="text-sm font-semibold">互联网络</h3>
							{#if isSharedNetworkMode}
								<p class="text-xs text-muted-foreground">网络命名空间通过以下方式共享<code class="px-1 py-0.5 rounded bg-muted">{containerData.HostConfig?.NetworkMode}</code>— 无法连接其他网络。</p>
					...
					<Tabs.Content value="mounts" class="space-y-4 overflow-auto">
								...
								{#each containerData.Mounts as mount}
									<div class="p-3 border border-border rounded-lg space-y-2">
										...
										<div class="grid grid-cols-1 lg:grid-cols-2 gap-2 text-xs">
											...
											<div>
												<p class="text-muted-foreground">目的地</p>
											...
											{#if mount.Propagation}
												<div>
													<p class="text-muted-foreground">传播</p>
						...
						{:else}
							<p class="text-sm text-muted-foreground">未配置任何挂载点。</p>
						{/if}
					</Tabs.Content>
					...
					<Tabs.Content value="env" class="space-y-4 overflow-auto">
						...
						{:else}
							<p class="text-sm text-muted-foreground">没有环境变量</p>
						{/if}
					</Tabs.Content>
						...
						{#if containerData.Config?.Labels && Object.keys(containerData.Config.Labels).length > 0}
							...
							<div class="flex items-center gap-2">
								...
								<Button
									...
									title={copiedAllLabels ? '已复制！' : 'Copy visible labels as key=value lines'}
								>
									{#if copiedAllLabels}
									...
									{#each visibleLabels as [key, value]}
										...
										<div class="text-xs p-2 rounded flex items-start gap-2 group {diverges ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-muted'}">
											...
											<button
												...
												title={copiedLabel === key ? '已复制！' : 'Copy label'}
											>
												{#if copiedLabel === key}
					...
					<Tabs.Content value="security" class="space-y-4 overflow-auto">
						...
						<div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
							{#if containerData.AppArmorProfile !== undefined}
								<div class="p-3 border border-border rounded-lg">
									<p class="text-xs text-muted-foreground mb-1">AppArmor配置文件</p>
							...
							{#if containerData.HostConfig?.SecurityOpt?.some((o: string) => o.startsWith('seccomp'))}
								<div class="p-3 border border-border rounded-lg">
									<p class="text-xs text-muted-foreground mb-1">安全计算</p>
									<code class="text-xs">
						...
						<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
							{#if containerData.HostConfig?.CapAdd?.length > 0}
								<div class="space-y-2">
									<h3 class="text-sm font-semibold text-green-600 dark:text-green-400">新增功能</h3>
							...
							{#if containerData.HostConfig?.CapDrop?.length > 0}
								<div class="space-y-2">
									<h3 class="text-sm font-semibold text-red-600 dark:text-red-400">能力下降</h3>
									<div class="flex flex-wrap gap-1">
						...
						{#if !containerData.HostConfig?.CapAdd?.length && !containerData.HostConfig?.CapDrop?.length && !containerData.HostConfig?.SecurityOpt?.length}
							<p class="text-sm text-muted-foreground">默认安全设置</p>
						{/if}
					</Tabs.Content>
					...
					<Tabs.Content value="resources" class="space-y-4 overflow-auto">
						...
						<div class="space-y-2">
							<h3 class="text-sm font-semibold flex items-center gap-2">
								<Settings2 class="w-4 h-4" />资源限制</h3>
							<div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
								<div class="p-3 border border-border rounded-lg">
									<p class="text-xs text-muted-foreground mb-1">CPU份额</p>
									<code class="text-sm">{containerData.HostConfig?.CpuShares || 'default'}</code>
								</div>
								<div class="p-3 border border-border rounded-lg">
									<p class="text-xs text-muted-foreground mb-1">CPU</p>
								...
								<div class="p-3 border border-border rounded-lg">
									<p class="text-xs text-muted-foreground mb-1">内存交换</p>
						...
						{#if containerData.HostConfig?.Ulimits?.length > 0}
							<div class="space-y-2">
								<h3 class="text-sm font-semibold">资源限制</h3>
						...
						{#if containerData.HostConfig?.Devices?.length > 0}
							<div class="space-y-2">
								<h3 class="text-sm font-semibold">设备</h3>
								<div class="space-y-1">
							...
							<div class="space-y-2">
								...
								<div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
									...
									{#if containerData.HostConfig?.DeviceRequests?.length > 0}
										...
										<div class="p-3 border border-border rounded-lg">
											<p class="text-xs text-muted-foreground mb-1">数数</p>
										...
										{#if req.Capabilities?.length > 0}
											<div class="p-3 border border-border rounded-lg col-span-full">
												<p class="text-xs text-muted-foreground mb-1">能力</p>
												<div class="flex flex-wrap gap-1.5">
						...
						<!-- Cgroup -->
						<div class="space-y-2">
							<h3 class="text-sm font-semibold">C组设置</h3>
							<div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
								<div class="p-2 bg-muted rounded">
								...
								<div class="p-2 bg-muted rounded">
									<p class="text-xs text-muted-foreground">C集团母公司</p>
									<code class="text-xs">{containerData.HostConfig?.CgroupParent || 'default'}</code>
								</div>
								<div class="p-2 bg-muted rounded">
									<p class="text-xs text-muted-foreground">Cgroupns 模式</p>
						...
						{#if healthConfig || healthState}
							<div class="flex flex-col flex-1 min-h-0 gap-4">
								...
								{#if healthState}
									<div class="shrink-0">
										...
										<div class="grid grid-cols-2 gap-3 text-sm">
											...
											<div>
												<p class="text-muted-foreground">连败</p>
								...
								{:else if healthConfig}
									<p class="text-sm text-muted-foreground">正在等待首次健康检查结果…</p>
								{/if}
							</div>
						{:else}
