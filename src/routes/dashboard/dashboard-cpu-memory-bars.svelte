...
{:else if !collectMetrics}
	...
	<div class="text-xs text-muted-foreground text-center py-1">指标收集已禁用</div>
{:else if !hasMetrics}
	<!-- No metrics available -->
	<div class="text-xs text-muted-foreground text-center py-1">暂无可用指标</div>
{:else if compact}
...
{:else}
	...
	<div class="space-y-2 pt-1 border-t border-border/50">
		...
		<div class="space-y-1">
			<div class="flex items-center justify-between text-xs">
				<span class="flex items-center gap-1 text-muted-foreground">
					<MemoryStick class="w-3 h-3" />内存</span>
