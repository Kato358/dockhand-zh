<script lang="ts">
	...
	async function testOidcConnection(configId: number) {
		...
		try {
			...
			oidcTestResult = data;
			if (data.success) {
				toast.success('OIDC连接成功');
			} else {
				toast.error(`OIDC 连接失败：${data.error}`);
			}
		} catch (error) {
			oidcTestResult = { success: false, error: '连接测试失败' };
			toast.error('OIDC连接测试失败');
		} finally {
			oidcTesting = null;
		}
	}
	...
	async function toggleOidcEnabled(config: OidcConfig) {
		try {
			...
			if (response.ok) {
				await fetchOidcConfigs();
				toast.success(`OIDC 提供程序 ${config.enabled ? '已禁用' : '已启用'}`);
			} else {
				toast.error('切换 OIDC 提供程序失败');
			}
		} catch (error) {
			...
			<div class="flex items-center justify-between">
				<div>
					<Card.Title class="text-sm font-medium flex items-center gap-2">
						<LogIn class="w-4 h-4" />单点登录提供商</Card.Title>
					<p class="text-xs text-muted-foreground mt-1">使用 Okta、Auth0、Azure AD 或 Google Workspace 等 OpenID Connect 提供商启用 SSO。</p>
				</div>
				{#if $canAccess('settings', 'edit')}
					<Button size="sm" onclick={() => openOidcModal(null)}>
						<Plus class="w-4 h-4" />添加提供商</Button>
				{/if}
			</div>
		</Card.Header>
		<Card.Content>
