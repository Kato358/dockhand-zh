				...
				<div class="flex items-center justify-between">
					<div>
						<Card.Title class="text-sm font-medium flex items-center gap-2">
							<Shield class="w-4 h-4" />角色</Card.Title>
						<p class="text-xs text-muted-foreground mt-1">定义具有细粒度权限的角色，并将其分配给用户以进行访问控制。</p>
					</div>
					{#if $canAccess('settings', 'edit')}
						<Button size="sm" onclick={() => openRoleModal(null)}>
							<Plus class="w-4 h-4" />添加角色</Button>
					{/if}
				</div>
			</Card.Header>
			<Card.Content>
										...
										<div class="flex flex-wrap items-center gap-1">
											...
											{#if role.environmentIds === null || role.environmentIds === undefined}
												<Badge variant="secondary" class="text-2xs gap-0.5 px-1 py-0 h-4">
													<Globe class="w-2.5 h-2.5" />全部</Badge>
								...
								{#if $canAccess('settings', 'edit')}
									<div class="flex items-center gap-1 flex-shrink-0">
										{#if role.isSystem}
											...
											<Button variant="ghost" size="sm" onclick={() => copyRole(role)} title="复制到新角色">
												<Copy class="w-4 h-4" />
											</Button>
										{:else}
