		...
		try {
			...
			if (data.success) {
				testResult = {
					id,
					success: true,
					message: `已连接！分支：${data.branch}，上次提交：${data.lastCommit}`
				};
...
<div class="space-y-4">
	<div class="flex justify-between items-center">
		<div>
			...
			<p class="text-sm text-muted-foreground">管理可用于部署编排的 Git 仓库</p>
		</div>
		{#if $canAccess('settings', 'edit')}
			<Button size="sm" onclick={() => openModal()}>
				<Plus class="w-4 h-4" />添加仓库</Button>
		{/if}
	</div>
		...
		<Card.Root>
			<Card.Content>
				<EmptyState
					icon={FolderGit2}
					title="未配置 Git 仓库。"
					description="添加一个仓库，以便在从 Git 部署编排时使用它。"
				/>
			</Card.Content>
		</Card.Root>
	{:else}
					...
					<div class="flex items-center gap-2 shrink-0">
						...
						{:else}
							<span class="flex items-center gap-1 text-xs text-muted-foreground" title="公共仓库">
								<Globe class="w-3 h-3" />
								<span class="hidden sm:inline">民众</span>
							</span>
						{/if}
						...
						{#if $canAccess('settings', 'edit')}
							<Button variant="ghost" size="icon" class="h-7 w-7" onclick={() => openModal(repo)} title="编辑库">
