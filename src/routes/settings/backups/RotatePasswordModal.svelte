	...
	async function submit() {
		...
		try {
			...
			if (res.ok && data.success) {
				toast.success(`密码已轮换，目标名称为“${destinationName}”。`);
				open = false;
				return;
			}
			if (data.dbOutOfSync) {
	...
	<Dialog.Content class="max-w-md">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-2">
				<KeyRound class="w-4 h-4" />轮换仓库密码</Dialog.Title>
			<Dialog.Description>更改 restic 加密密码<strong>{destinationName}</strong>. The current
		...
		<div class="space-y-3 py-2">
			...
			<div class="space-y-1">
				...
				{#if confirmPassword.length > 0 && !passwordsMatch}
					<p class="text-xs text-destructive">密码不匹配</p>
				{/if}
				{#if newPassword.length > 0 && newPassword === currentPassword}
					<p class="text-xs text-destructive">新密码必须与当前密码不同</p>
				{/if}
			</div>
				...
				<div class="rounded border border-destructive/40 bg-destructive/10 p-3 text-sm">
					<div class="flex items-start gap-2">
						...
						<div>
							...
							{#if dbOutOfSync}
								<p class="text-muted-foreground mt-2">打开<strong>编辑目标</strong> and manually set the password to the new
