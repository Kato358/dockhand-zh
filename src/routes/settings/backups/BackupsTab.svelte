	...
	async function deleteDestination(id: number) {
		try {
			...
			if (response.ok) {
				...
				toast.error(data.error || '删除目标失败');
			}
		} catch { toast.error('删除目标失败'); }
	}
...
<div class="space-y-4">
	...
	<div class="flex items-center gap-3 p-3 border rounded-md bg-muted/20">
		<Label class="text-xs shrink-0 flex items-center gap-1.5">备份辅助映像<Tooltip.Provider delayDuration={200}>
				<Tooltip.Root>
					<Tooltip.Trigger>
						<CircleHelp class="w-3 h-3 text-muted-foreground/70 cursor-help" />
					</Tooltip.Trigger>
					<Tooltip.Portal>
						<Tooltip.Content side="bottom" sideOffset={4} class="!w-80 text-xs">用于备份/还原的 restic Docker 镜像。首次运行时自动拉取。针对私有镜像仓库进行了更改。</Tooltip.Content>
					</Tooltip.Portal>
				</Tooltip.Root>
			</Tooltip.Provider>
		</Label>
	...
	<div class="flex justify-between items-center">
		<div class="flex items-center gap-3">
			<SearchInput bind:value={searchQuery} placeholder="筛选目的地…" class="h-8 w-64 text-sm" />
			<Select.Root type="single" value={statusFilter} onValueChange={(v) => statusFilter = (v as typeof statusFilter) ?? 'all'}>
				<Select.Trigger class="h-8 w-40 text-sm">
					{statusFilter === 'success' ? '已初始化' : statusFilter === 'needs_init' ? '需要初始化' : statusFilter === 'failed' ? '失败' : '所有状态'}
				</Select.Trigger>
				<Select.Content>
					...
					<Select.Item value="needs_init"><AlertCircle class="w-3.5 h-3.5 text-amber-500 mr-1.5 inline" />需要初始化</Select.Item>
					<Select.Item value="failed"><XCircle class="w-3.5 h-3.5 text-destructive mr-1.5 inline" />失败</Select.Item>
				</Select.Content>
			</Select.Root>
		...
		<div class="flex gap-2">
			{#if $canAccess('backups', 'manage')}
				<Button size="sm" onclick={() => openModal()}>
					<Plus class="w-4 h-4 mr-1" />添加目的地</Button>
			{/if}
	...
	{#if loading && destinations.length === 0}
		<p class="text-muted-foreground text-sm">正在加载备份目标位置…</p>
	{:else if destinations.length === 0}
		<EmptyState
			icon={Archive as unknown as Component}
			title="没有备份目标"
			description="添加备份目标，即可开始保护您的容器数据"
		/>
	{:else}
		<DataGrid
				...
				{:else if column.id === 'stats'}
					...
					{#if stat}
						<div class="flex items-center gap-2 text-xs text-muted-foreground">
							<span class="flex items-center gap-0.5" title="总尺寸"><HardDrive class="w-3 h-3" />{formatBytes(stat.totalSize)}</span>
							<span class="flex items-center gap-0.5" title="文件"><FileStack class="w-3 h-3" />{stat.totalFiles}</span>
							<span class="flex items-center gap-0.5" title="快照"><Camera class="w-3 h-3" />{stat.snapshots}</span>
						</div>
				...
				{:else if column.id === 'status'}
					...
					{:else}
						<div class="flex items-center gap-1.5">
							...
							<span class="text-xs text-muted-foreground">未经测试</span>
						</div>
					{/if}
				{:else if column.id === 'actions'}
					...
					<div class="flex items-center justify-end gap-0.5" onclick={(e) => e.stopPropagation()}>
						{#if dest.lastTestStatus === 'success'}
							...
							<button type="button" class="p-0.5 rounded hover:bg-muted transition-colors opacity-70 hover:opacity-100" onclick={() => runRepoTask(dest.id, 'check')} disabled={runningTask?.destId === dest.id} title="检查完整性">
							...
							<button type="button" class="p-0.5 rounded hover:bg-muted transition-colors opacity-70 hover:opacity-100" onclick={() => verifyDestination(dest)} title="验证数据完整性">
							...
							<ConfirmPopover
								...
								action="解锁" itemType="repository" itemName={dest.name} confirmText="解锁" position="left" title="解锁仓库"
							...
							<ConfirmPopover
								...
								action="清理" itemType="unused data from" itemName={dest.name} confirmText="清理" variant="destructive" position="left" title="删除未使用的数据"
							...
							<ConfirmPopover
								...
								action="修复" itemType="index for" itemName={dest.name} confirmText="修复" variant="destructive" position="left" title="修复索引"
						...
						{#if dest.lastTestStatus === 'needs_init'}
							<button type="button" class="p-0.5 rounded hover:bg-muted transition-colors text-amber-500 hover:text-amber-600" onclick={() => initDestination(dest.id)} disabled={initializingId === dest.id} title="初始化仓库">
	...
	<Dialog.Content class="max-w-6xl w-[calc(100vw-4rem)] h-[88vh] flex flex-col overflow-hidden">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-2">
				<FolderOpen class="w-5 h-5" />仓库快照<svelte:component this={getRepoTypeIcon(browseDestRepo)} class="w-4 h-4" />
		...
		<div class="flex-1 min-h-0">
			...
			{:else if browseSnapshots.length === 0}
				<div class="flex h-full items-center justify-center"><p class="text-sm text-muted-foreground">此仓库中没有快照。</p></div>
			{:else}
				<DataGrid
					data={browseGroups}
					keyField="key"
								...
								<Table.Root>
									<Table.Header class="sticky top-0 z-10 bg-background">
										<Table.Row>
											...
											<Table.Head class="py-1.5 text-xs" style="padding-left:8px">已创建</Table.Head>
